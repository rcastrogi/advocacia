...
def list_section_fields(slug):
    ...
    for i, field in enumerate(fields, 1):
        print(f"{i:2}. {field.get('name'):30} | tipo: {field.get('type'):12} | {field.get('label')}")
...
def update_cpf_fields_with_link(representante_id):
    ...
    # Seções de partes
    target_slugs = ["autor", "autor-peticionario", "reu", "reu-acusado", 
...
def main():
    ...
    with app.app_context():
        # Seções de partes
        slugs = ["autor", "autor-peticionario", "reu", "reu-acusado", 
                 "terceiro-interessado", "testemunha", "representante-legal"]
