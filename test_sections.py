...
try:
    ...
    if response.status_code == 200:
        ...
        # Testar login
        login_data = {'email': 'admin@petitio.com', 'password': 'admin123', 'submit': 'Entrar'}
        response = session.post(f'{base_url}/auth/login', data=login_data, allow_redirects=True, timeout=10)
        ...
        if 'admin' in response.text.lower():
            ...
            if response.status_code == 200 and 'Seções de Petição' in response.text:
                ...
                print('🎉 Você pode acessar em: http://127.0.0.1:5000/admin/petitions/sections')
            else:
