    ...
    if cpf_cnpj_fields:
        ...
        for cf in cpf_cnpj_fields:
            ...
            if linked and trigger:
                success.append(
                    f"  - [{cf['section_slug']}] {cf['field_name']} -> secao {linked} (trigger: {trigger})"
                )
            elif linked:
                warnings.append(
                    f"  - [{cf['section_slug']}] {cf['field_name']} -> secao {linked} (SEM trigger)"
                )
            else:
                warnings.append(
                    f"  - [{cf['section_slug']}] {cf['field_name']} (SEM vinculo)"
                )
    else:
        ...
        for cf in cpf_cnpj_fields:
            ...
            if linked_id:
                if linked_id == rep_section.id:
                    ...
                    warnings.append(
                        f"  - {cf['field_name']}: vinculado a secao {linked_id} (esperado: {rep_section.id})"
                    )
