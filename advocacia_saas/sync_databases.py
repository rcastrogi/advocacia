        ...
        with local_engine.begin() as conn:
            for i, item in enumerate(render_items, 1):
                ...
                try:
                    ...
                    print(f"  [{i}/{len(render_items)}] ID {item['id']}: {item['title'][:40]}")
    ...
    try:
        ...
        with render_engine.begin() as conn:
            for i, item in enumerate(local_items, 1):
                ...
                set_clause = ", ".join([f"{k}=%s" for k in item_copy.keys() if k != 'id'])
                update_sql = f"""
                    UPDATE roadmap_item 
                    SET {set_clause}
                    WHERE id = %s
                ...
                try:
                    ...
                    print(f"  [{i}/{len(local_items)}] ID {item['id']}: {item['title'][:40]}")
                except Exception as e:
                    print(f"  [!] Erro ao sincronizar ID {item['id']}: {str(e)}")
        ...
        if common:
            ...
            if differences:
                ...
                for diff in differences:
                    ...
                    print(f"      Render: {diff['render_status']} → Local: {diff['local_status']}")
            else:
