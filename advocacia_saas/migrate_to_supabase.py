...
print(
    f"URL: postgresql://{SUPABASE_USER}:****@{SUPABASE_HOST}:{SUPABASE_PORT}/{SUPABASE_DB}"
)
...
def test_connection(engine):
    ...
    try:
        with engine.connect() as conn:
            ...
            print(
                f"  ✓ Conectado: PostgreSQL {version.split(',')[0].replace('PostgreSQL ', '')}"
            )
            return True
    except Exception as e:
    ...
    with app.app_context():
        ...
        for table_name in insert_order:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    for fmt in [
                                        ...
                                        try:
                                            clean_row[key] = datetime.strptime(
                                                value, fmt
                                            )
                                            break
                                        except ValueError:
            ...
            try:
                db.session.commit()
                print(
                    f"✓ {inserted} registros" + (f" ({errors} erros)" if errors else "")
                )
            except Exception as e:
                db.session.rollback()
