...
from app.models import PetitionSection, PetitionModel, PetitionModelSection
        ...
        try:
            ...
            for section in sections:
                ...
                for field in section.fields_schema:
                    ...
                    if any(addr in field_name for addr in ['cep', 'street', 'endereco', 'rua', 'logradouro', 'bairro', 'neighborhood', 'cidade', 'city', 'estado', 'uf', 'complement', 'complemento']):
                ...
                if has_address_fields:
                    ...
                    for i, field in enumerate(section.fields_schema):
                        ...
                        if field_name == 'endereco' and field.get('type') in ['textarea', 'text']:
                            print(f"      ⚠️  Campo 'endereco' em formato combinado detectado!")
                            # Este campo deveria ser separado
                            # Mas vamos manter como está por enquanto e apenas documentar
                    
                    # Mostrar campos que faltam
                    expected_address_fields = {'cep', 'street', 'number', 'city', 'neighborhood', 'state', 'complement'}
                    ...
                    for field in section.fields_schema:
                        ...
                        elif 'street' in field_name or 'rua' in field_name or 'logradouro' in field_name:
                            existing_address_names.add('street')
                        elif 'number' in field_name or 'número' in field_name:
                            existing_address_names.add('number')
                        elif 'city' in field_name or 'cidade' in field_name or 'municipio' in field_name:
                        ...
                        elif 'state' in field_name or 'uf' in field_name or 'estado' in field_name:
            ...
            address_section = PetitionSection.query.filter_by(slug='endereco-localizacao').first()
            ...
            if address_section and address_section.fields_schema:
                ...
                for i, field in enumerate(address_section.fields_schema, 1):
                    print(f"  {i}. {field.get('name')}: {field.get('label')} ({field.get('type')})")
                    if field.get('size'):
