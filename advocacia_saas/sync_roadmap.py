...
from app.models import RoadmapItem, RoadmapCategory
...
def export_roadmap_snapshot(name_suffix=""):
    ...
    with app.app_context():
        ...
        snapshot = {
            ...
            'environment': 'render' if 'render.com' in os.getenv('DATABASE_URL', '') else 'local',
        ...
        for item in items:
            status = item.status
            snapshot['statistics']['by_status'][status] = snapshot['statistics']['by_status'].get(status, 0) + 1
            
            cat_name = item.category.name if item.category else 'Sem Categoria'
            snapshot['statistics']['by_category'][cat_name] = snapshot['statistics']['by_category'].get(cat_name, 0) + 1
        
        completed = snapshot['statistics']['by_status'].get('completed', 0)
        total = len(items)
        snapshot['statistics']['total_progress'] = round((completed / total * 100), 2) if total > 0 else 0
        
        # Salvar snapshot
        filename = Path('roadmap_snapshots') / f"snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}{name_suffix}.json"
...
def compare_snapshots(old_snapshot, new_snapshot):
    ...
    progress_change = new_snapshot['statistics']['total_progress'] - old_snapshot['statistics']['total_progress']
        ...
        if item_id in old_items:
            ...
            if old_item['status'] != new_item['status']:
                changes.append({
                    ...
                    'old': old_item['status'],
                    'new': new_item['status'],
                    'category': new_item['category']['name'] if new_item['category'] else 'N/A'
                })
            
            # Verificar mudanças de datas
            if old_item.get('actual_completion_date') != new_item.get('actual_completion_date'):
    ...
    if changes:
        for change in changes:
            if change['type'] == 'status':
                ...
                print(f"   Status: {status_map.get(change['old'], change['old'])} → {status_map.get(change['new'], change['new'])}")
    ...
    # Itens sem mudança
    unchanged = sum(1 for item_id in new_items.keys() 
                   if item_id in old_items and 
                   old_items[item_id]['status'] == new_items[item_id]['status'])
    print(f"✓ Itens sem mudanças: {unchanged}")
    
    print()
    
    # Novos itens
    new_items_list = [item_id for item_id in new_items.keys() if item_id not in old_items]
    if new_items_list:
    ...
    # Itens removidos
    removed_items = [item_id for item_id in old_items.keys() if item_id not in new_items]
    if removed_items:
...
def main():
    ...
    snapshots = sorted(snapshots_dir.glob('snapshot_*.json')) if snapshots_dir.exists() else []
