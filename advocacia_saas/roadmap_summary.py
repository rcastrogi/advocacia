...
class Colors:
    ...
    CRITICAL = (
        "\033[38;2;255;255;255m\033[48;2;220;0;0m"  # Branco fundo vermelho intenso
    )
    ...
    # Status com cores
    COMPLETED = (
        "\033[38;2;255;255;255m\033[48;2;34;139;34m"  # Branco fundo verde escuro
    )
...
try:
    ...
    for difficulty, count, avg_impact in cursor.fetchall():
        print(
            f"   {difficulty:15s}: {count:2d} items (Impact médio: {avg_impact:.1f}/5)"
        )
    ...
    for month, year, total_planned, completed in cursor.fetchall():
        if month:  # Evitar None
            ...
            print(
                f"   {month_name}/{year}: {completed or 0} completas + {remaining} planejadas"
            )
    ...
    print("\n" + "=" * 100)
    print(
        f"Relatório gerado em: {datetime.now().strftime('%d/%m/%Y às %H:%M:%S')}".center(
            100
        )
    )
