...
class TestAISystem:
    ...
    def test_ai_generation_requires_credits(self, client, db_session):
        ...
        # Login
        client.post("/auth/login", data={"email": "test@example.com", "password": "Test123!", "submit": "Entrar"})
        ...
        assert "créditos" in data.get("error", "").lower() or "credits" in data.get("error", "").lower()
    ...
    def test_ai_generation_success(self, client, db_session):
        ...
        with patch('app.ai.routes.ai_service.generate_section') as mock_generate:
            mock_generate.return_value = ("Texto gerado pela IA", {
                "model": "gpt-4o-mini",
                "tokens_input": 50,
                "tokens_output": 100,
                "tokens_total": 150,
    ...
    def test_ai_generation_error_handling(self, client, db_session):
        ...
        with patch('app.ai.routes.ai_service.generate_section') as mock_generate:
            ...
            generation = AIGeneration.query.filter_by(user_id=user.id, status="failed").first()
            assert generation is not None
    ...
    def test_credits_dashboard_access(self, client, db_session):
        ...
        assert response.status_code == 200
        assert "créditos".encode('utf-8') in response.data.lower() or b"credits" in response.data.lower()

    @pytest.mark.skip(reason="Complex Mercado Pago integration test - requires external service setup")
    ...
    def test_master_user_bypass_credits(self, client, db_session):
        ...
        with patch('app.ai.routes.ai_service.generate_section') as mock_generate:
            mock_generate.return_value = ("Texto gerado pelo admin", {
                "model": "gpt-4o-mini", 
                "tokens_total": 100
            })
    ...
    def test_ai_generation_rate_limiting(self, client, db_session):
        ...
        with patch('app.ai.routes.ai_service.generate_section') as mock_generate:
            mock_generate.return_value = ("Texto gerado", {
                "model": "gpt-4o-mini", 
                "tokens_total": 100
            })
    ...
    def test_ai_usage_statistics(self, client, db_session):
        ...
        assert "gerações".encode('utf-8') in response.data.lower() or b"generations" in response.data.lower()