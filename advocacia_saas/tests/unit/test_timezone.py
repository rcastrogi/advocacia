...
class TestTimezoneSystem:
    ...
    def test_local_datetime_filter(self, app):
        ...
        with app.app_context():
            ...
            result_tokyo = app.jinja_env.filters['local_datetime'](utc_dt, user_timezone="Asia/Tokyo")
    ...
    def test_local_date_filter(self, app):
        ...
        with app.app_context():
            ...
            result_tokyo = app.jinja_env.filters['local_date'](utc_dt, user_timezone="Asia/Tokyo")
            assert result_tokyo == "24/12/2025"

            # Testar mudança de data devido ao timezone (exemplo: Nova York)
            utc_dt_early = datetime.datetime(2025, 12, 24, 5, 0, 0, tzinfo=pytz.UTC)  # 5:00 UTC
            result_ny = app.jinja_env.filters['local_date'](utc_dt_early, user_timezone="America/New_York")
    ...
    def test_datetime_filter_custom_format(self, app):
        ...
        with app.app_context():
            ...
            result = app.jinja_env.filters['local_datetime'](utc_dt, format_string='%Y-%m-%d %H:%M:%S')
            assert "2025-12-24 09:00:00" in result

            # Formato personalizado para date
            result_date = app.jinja_env.filters['local_date'](utc_dt, format_string='%Y-%m-%d')
