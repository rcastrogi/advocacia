...
def upgrade():
    ...
    op.add_column(
        "user", sa.Column("two_factor_method", sa.String(length=20), nullable=True)
    )
    op.add_column("user", sa.Column("totp_secret", sa.String(length=32), nullable=True))
    op.add_column(
        "user", sa.Column("two_factor_backup_codes", sa.Text(), nullable=True)
    )
    op.add_column(
        "user", sa.Column("two_factor_last_used", sa.DateTime(), nullable=True)
    )
