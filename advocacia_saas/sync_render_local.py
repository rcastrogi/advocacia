        ...
        with app.app_context():
            ...
            for i, local_item in enumerate(local_items, 1):
                try:
                    ...
                    render_item = (
                        render_session.query(RoadmapItem)
                        .filter_by(id=local_item.id)
                        .first()
                    )
                    ...
                    if render_item:
                        ...
                        render_item.planned_completion_date = (
                            local_item.planned_completion_date
                        )
                        render_item.actual_completion_date = (
                            local_item.actual_completion_date
                        )
                        ...
                        updated += 1
                        print(
                            f"  [{i}/{len(local_items)}] ✓ ID {local_item.id}: {local_item.title[:40]}"
                        )
                    else:
                        # Inserir novo
                        ...
                        inserted += 1
                        print(
                            f"  [{i}/{len(local_items)}] + ID {local_item.id}: {local_item.title[:40]} (novo)"
                        )
...
def sync_render_to_local():
    ...
    try:
        ...
        with app.app_context():
            ...
            render_items = (
                render_session.query(RoadmapItem).order_by(RoadmapItem.id).all()
            )
            ...
            for i, render_item in enumerate(render_items, 1):
                try:
                    ...
                    if local_item:
                        ...
                        updated += 1
                        print(
                            f"  [{i}/{len(render_items)}] ✓ ID {render_item.id}: {render_item.title[:40]}"
                        )
                    else:
                        new_item = RoadmapItem(
                            id=render_item.id,
                        ...
                        inserted += 1
                        print(
                            f"  [{i}/{len(render_items)}] + ID {render_item.id}: {render_item.title[:40]} (novo)"
                        )
