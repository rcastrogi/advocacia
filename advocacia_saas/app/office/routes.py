...
from app.models import Office, OfficeInvite, User, OFFICE_ROLES
...
def office_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.office_id:
            flash("Você precisa criar ou participar de um escritório para acessar esta funcionalidade.", "warning")
...
def office_owner_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not current_user.is_office_owner():
            flash("Apenas o proprietário do escritório pode realizar esta ação.", "danger")
            return redirect(url_for("office.dashboard"))
        return f(*args, **kwargs)
    return decorated_function
...
def members():
    ...
    pending_invites = office.invites.filter_by(status="pending").order_by(OfficeInvite.created_at.desc()).all()
    
    # Buscar inviters para os convites pendentes
    inviter_ids = set(invite.invited_by_id for invite in pending_invites)
    inviter_users = {u.id: u for u in User.query.filter(User.id.in_(inviter_ids)).all()} if inviter_ids else {}
...
def invite_member():
    ...
    if not office.can_add_member():
        flash(f"Limite de {office.get_max_members()} membros atingido. Faça upgrade do plano para adicionar mais.", "warning")
...
def change_member_role(member_id):
    ...
    if new_role in OFFICE_ROLES:
        ...
        db.session.commit()
        flash(f"Função de {member.full_name or member.username} alterada para {OFFICE_ROLES[new_role]['name']}.", "success")
    else:
...
def remove_member(member_id):
    ...
    if member.id == current_user.id:
        flash("Você não pode se remover do escritório. Use 'Sair do Escritório' ao invés.", "warning")
        return redirect(url_for("office.members"))
...
def cancel_invite(invite_id):
    ...
    invite = OfficeInvite.query.filter_by(id=invite_id, office_id=office.id, status="pending").first()
...
def accept_invite_page(token):
    ...
    if current_user.office_id:
        flash("Você já pertence a um escritório. Saia primeiro para aceitar este convite.", "warning")
        return redirect(url_for("office.dashboard"))
...
def transfer_ownership():
    ...
    form.new_owner_id.choices = [(m.id, f"{m.full_name or m.username} ({m.email})") for m in eligible_members]
    
    if not eligible_members:
        flash("Não há outros membros elegíveis para transferir a propriedade.", "warning")
        return redirect(url_for("office.dashboard"))
    ...
    if form.validate_on_submit():
        ...
        if new_owner and new_owner.office_id == office.id:
            ...
            flash(f"Propriedade transferida para {new_owner.full_name or new_owner.username}.", "success")
            return redirect(url_for("office.dashboard"))
        else:
...
def leave_office():
    ...
    if current_user.is_office_owner():
        flash("Você é o proprietário. Transfira a propriedade antes de sair ou exclua o escritório.", "danger")
        return redirect(url_for("office.dashboard"))
