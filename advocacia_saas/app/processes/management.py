...
from app import db
from app.models import Process, ProcessMovement, ProcessCost, ProcessAttachment

bp = Blueprint("process_management", __name__, url_prefix="/processes")

# Configurações para upload
UPLOAD_FOLDER = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "uploads", "process_attachments")
...
def download_attachment(attachment_id):
    """Download de anexo"""
    attachment = ProcessAttachment.query.filter_by(id=attachment_id, user_id=current_user.id).first_or_404()
...
def api_movements(process_id):
    ...
    movements = ProcessMovement.query.filter_by(process_id=process_id).order_by(
