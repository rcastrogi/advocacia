...
def check_and_send_notifications():
    ...
    for process in processes_with_deadlines:
        ...
        elif days_until <= 3 and days_until > 0 and not _notification_exists(process.id, "deadline_urgent", today):
            if send_deadline_notification(process, "deadline_urgent", days_until):
                notifications_sent += 1

        # Aviso de prazo (7 dias)
        elif days_until <= 7 and days_until > 3 and not _notification_exists(process.id, "deadline_warning", today):
