...
from app.models import RoadmapItem, UserPlan, BillingPlan
...
def cast_vote():
    ...
    if not data or 'roadmap_item_id' not in data:
        return jsonify({
            'success': False,
            'error': 'roadmap_item_id é obrigatório'
        }), 400
...
def get_votes_leaderboard():
    ...
    results = db.session.query(
        ...
        RoadmapVote.billing_period == period
    ).group_by(
        RoadmapItem.id,
        RoadmapItem.title,
        RoadmapItem.status
...
def get_my_votes():
    ...
    votes = RoadmapVote.query.filter_by(
        user_id=current_user.id,
        billing_period=period
    ).join(RoadmapItem).all()
