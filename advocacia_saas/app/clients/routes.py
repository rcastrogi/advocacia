...
from app.office.utils import (
    can_access_record,
    filter_by_office_member,
    get_office_user_ids,
)
...
def check_duplicate_cpf_cnpj(
    cpf_cnpj: str, exclude_client_id: int = None
) -> Client | None:
    """
...
def edit(id):
    ...
    if form.validate_on_submit():
        ...
        if form.cpf_cnpj.data != client.cpf_cnpj:
            existing_client = check_duplicate_cpf_cnpj(
                form.cpf_cnpj.data, exclude_client_id=client.id
            )
            if existing_client:
                flash(
