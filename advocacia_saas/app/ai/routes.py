...
from app.models import (
    UserCredits, CreditTransaction, CreditPackage, AIGeneration, 
    PetitionType, User
)
from app.services.ai_service import ai_service, CREDIT_COSTS
...
def record_transaction(user_id, transaction_type, amount, description, 
                       package_id=None, generation_id=None, payment_intent_id=None):
...
def record_ai_generation(user_id, generation_type, credits_used, metadata, 
                         petition_type_slug=None, section_name=None,
                         input_data=None, output_content=None, status='completed',
                         error_message=None):
...
def credits_dashboard():
    ...
    return render_template('ai/credits_dashboard.html',
...
def generations_history():
    ...
    page = request.args.get('page', 1, type=int)
    generations = AIGeneration.query.filter_by(user_id=current_user.id)\
...
def api_generate_full_petition():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Erro ao gerar petição: {str(e)}'
        }), 500
...
def api_improve_text():
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'error': f'Erro ao melhorar texto: {str(e)}'
        }), 500
