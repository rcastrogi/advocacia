...
from app import db
from app.models import CreditTransaction, Feature, User, UserCredits
...
class CreditsService:
        ...
        for row in users_with_credits:
            try:
                ...
                last_renewal = CreditTransaction.query.filter(
                    ...
                    db.func.to_char(CreditTransaction.created_at, "YYYY-MM")
                    == current_month,
                ).first()
    ...
    def check_and_deduct_credits(user, amount=1):
        ...
        if user.credits.balance < amount:
            return (
                False,
                f"Créditos insuficientes. Saldo: {user.credits.balance}",
