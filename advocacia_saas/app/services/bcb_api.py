...
def calcular_fator_correcao(indice: str, data_inicial: datetime, data_final: datetime) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "fator_correcao": float(fator_acumulado.quantize(Decimal("0.000001"), ROUND_HALF_UP)),
        "percentual_correcao": float(percentual_correcao.quantize(Decimal("0.01"), ROUND_HALF_UP)),
        "periodo": {
            "inicio": data_inicial.strftime("%d/%m/%Y"),
            "fim": data_final.strftime("%d/%m/%Y"),
            "meses": len(valores_usados)
        },
        "valores_mensais": valores_usados[-12:] if len(valores_usados) > 12 else valores_usados,  # Últimos 12 meses
...
def _calcular_fallback(indice: str, serie_info: dict, meses: float) -> Dict[str, Any]:
    ...
    return {
        ...
        "percentual_correcao": float(percentual.quantize(Decimal("0.01"), ROUND_HALF_UP)),
        "periodo": {
            "inicio": "",
            "fim": "",
            "meses": int(meses)
