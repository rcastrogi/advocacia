...
from app import db
from app.models import CalendarEvent, Client, Deadline, Document, Message, Process, User
...
Como posso ajudar?
    ...
    def _handle_next_deadline(self) -> Tuple[str, Optional[Dict]]:
        ...
        if not next_deadline:
            return (
                "✅ Você não tem prazos pendentes no momento. Que ótima notícia!",
                None,
            )
...
Quer ver todos os prazos? Pergunte "Quais são meus prazos?"
    ...
    def _handle_all_deadlines(self) -> Tuple[str, Optional[Dict]]:
        ...
        for i, d in enumerate(deadlines, 1):
            ...
            lines.append(
                f"   📆 {d.deadline_date.strftime('%d/%m/%Y')} ({days_until} dias)"
            )
    ...
    def _handle_documents(self) -> Tuple[str, Optional[Dict]]:
        ...
        recent_docs = (
            Document.query.filter_by(client_id=self.client.id)
        ...
        if doc_count == 0:
            return (
                "📄 Você ainda não tem documentos no portal. Use a opção 'Enviar Documento' para adicionar.",
                None,
            )
        ...
        if recent_docs:
            ...
            for doc in recent_docs:
                ...
                lines.append(
                    f"   • {doc.filename or doc.title or 'Documento'} ({date_str})"
                )
    ...
    def _handle_talk_to_lawyer(self) -> Tuple[str, Optional[Dict]]:
        ...
        if not self.lawyer:
            return (
                "❌ Não foi possível identificar seu advogado. Entre em contato com o escritório.",
                None,
            )
...
Qual sua dúvida? Posso tentar ajudar!
        ...
        for i, slot in enumerate(available_slots[:3], 1):
            ...
            lines.append(
                f"📅 **Opção {i}:** {day_name}, {slot.strftime('%d/%m/%Y às %H:%M')}"
            )
    ...
    def _get_available_slots(self) -> List[datetime]:
        ...
        for event in existing_events:
            ...
            busy_slots.add(
                event.start_datetime.replace(minute=0, second=0, microsecond=0)
            )
...
def process_client_message(
    client: Client, message_text: str
