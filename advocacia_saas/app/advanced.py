...
from app.models import (
    Process, ProcessMovement, ProcessCost, ProcessAttachment,
    CalendarEvent, ProcessAutomation, ProcessReport,
    Notification
)
...
def adjust_color_brightness(hex_color, factor):
    ...
    if factor < 0:
        return '#8b0000' if hex_color == '#dc3545' else '#b8860b' if hex_color == '#ffc107' else '#00008b' if hex_color == '#007bff' else '#006400'
    return hex_color
...
def new_calendar_event():
    ...
    return render_template('advanced/new_event.html',
                         processes=processes,
                         clients=clients)
...
def edit_calendar_event(event_id):
    ...
    if request.method == 'POST':
        try:
            ...
            event.process_id = int(request.form.get('process_id')) if request.form.get('process_id') else None
            event.client_id = int(request.form.get('client_id')) if request.form.get('client_id') else None
    ...
    return render_template('advanced/edit_event.html',
                         event=event,
                         processes=processes,
                         clients=clients)
...
def reports():
    ...
    reports_list = ProcessReport.query.filter_by(user_id=current_user.id).order_by(
        ProcessReport.created_at.desc()
    ).all()
