...
class UserOnboarding(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'first_tour_at': self.first_tour_at.isoformat() if self.first_tour_at else None,
            'last_tour_at': self.last_tour_at.isoformat() if self.last_tour_at else None,
...
def mark_tour_complete(tour_name):
    ...
    return jsonify({
        'success': False,
        'message': f'Tour "{tour_name}" não encontrado'
    }), 400
...
def should_show_tour():
    ...
    should_show = (
        not onboarding.skip_tours and
        not onboarding.first_tour_at and
        onboarding.tour_completion_rate == 0
    )
    ...
    return jsonify({
        ...
        'is_new_user': (datetime.now(timezone.utc) - current_user.created_at).days < 1
    })
