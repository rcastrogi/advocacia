...
from app.models import User, RoadmapItem
...
def get_week_start():
    ...
    monday = now - timedelta(days=days_since_monday, hours=now.hour, minutes=now.minute, seconds=now.second, microseconds=now.microsecond)
    return monday
...
def generate_newsletter_html(user_email):
    ...
    top_voted_query = db.session.query(
        RoadmapItem.id,
        RoadmapItem.title,
        RoadmapItem.status,
        ...
        RoadmapVote,
        RoadmapItem.id == RoadmapVote.roadmap_item_id
    ).filter(
        RoadmapVote.billing_period == current_month
    ).group_by(
        RoadmapItem.id,
        RoadmapItem.title,
        RoadmapItem.status
    ).order_by(
        db.func.sum(RoadmapVote.votes_spent).desc()
    ).limit(5).all()
    ...
    if top_voted_query:
        for rank, (item_id, title, status, total_votes) in enumerate(top_voted_query, 1):
