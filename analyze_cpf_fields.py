    ...
    for section in sections:
        ...
        for field in fields:
            ...
            if field_type in ["cpf", "cnpj", "cpf_cnpj"]:
                ...
                print(f"  Linked Section ID: {field.get('linked_section_id', 'Nao configurado')}")
                print(f"  Linked Trigger: {field.get('linked_section_trigger', 'Nao configurado')}")
...
def show_representante_section():
    ...
    for i, field in enumerate(section.fields_schema or [], 1):
        print(f"  {i}. {field.get('name')} ({field.get('type')}) - {field.get('label')}")
...
def update_sections_cpf_to_cpf_cnpj(representante_section_id):
    ...
    for slug in target_sections:
        ...
        for field in fields:
            ...
            if field_type == "cpf" and any(kw in field_name.lower() for kw in ["cpf", "documento"]):
                field["type"] = "cpf_cnpj"
                ...
                print(f"  Vinculado a secao de representante (ID: {representante_section_id})")
...
def update_representante_section_fields():
    ...
    section.fields_schema = new_fields
    section.description = "Qualificacao do representante legal para pessoas juridicas (CNPJ)"
    db.session.add(section)
    db.session.commit()
...
def main():
    ...
    with app.app_context():
        ...
        print("\n" + "-" * 80)
        confirm = input("Deseja converter campos CPF para CPF/CNPJ nas secoes de partes? (s/n): ")
        if confirm.lower() == 's':
