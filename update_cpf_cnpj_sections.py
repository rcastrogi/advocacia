...
def update_cpf_fields_to_cpf_cnpj(representante_section_id):
    ...
    for section in sections:
        ...
        for field in fields:
            ...
            should_convert = (
                field_type == "cpf" and 
                any(keyword in field_name.lower() for keyword in ["autor", "reu", "requerente", "requerido", "parte", "cliente"])
            )
            ...
            if should_convert:
                ...
                field["linked_section_trigger"] = "cnpj"  # Mostrar seção quando for CNPJ
                modified = True
                print(f"  -> Campo '{field_name}' convertido para cpf_cnpj na secao '{section.name}'")
                updated_count += 1
    ...
    for section in sections:
        ...
        if cpf_fields:
            ...
            for f in cpf_fields:
                ...
                print(f"      - {f.get('name')}: tipo={f.get('type')}, linked_section={linked}")
...
def validate_implementation():
    ...
    if not representante:
        ...
        for rf in required_fields:
            if not any(f.get("name") == rf for f in fields):
                errors.append(f"Campo obrigatorio '{rf}' nao encontrado na secao de representante")
            else:
    ...
    if cpf_cnpj_fields:
        ...
        for cf in cpf_cnpj_fields:
            ...
            if not linked:
                warnings.append(f"Campo {cf['field']} em {cf['section']} sem linked_section_id")
    else:
